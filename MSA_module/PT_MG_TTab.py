...
config = {
    'batch_size': 4,
    'sequence_file_train': 'para_train_ab_sequences_3000.npz',
    'data_file_train': 'para_train_ab_interfaces_3000.npz',
    'edge_file_train': 'para_train_ab_edges_3000.npz',
    'sequence_file_val': 'para_val_ab_sequences_3000.npz',
    'data_file_val': 'para_val_ab_interfaces_3000.npz',
    'edge_file_val': 'para_val_ab_edges_3000.npz',
    'max_len': 3000,
    'vocab_size': 23,
    'embed_dim': 256,
    'num_heads': 16,
...
for epoch in range(config['num_epochs']):
    ...
    for i, (padded_edges, sequence_tensor, pt_tensor) in enumerate(train_loader):
        ...
        if pt_tensor.isnan().any() or pt_tensor.isinf().any():
            print("Skipping batch with NaN/Inf in target tensor.")
            continue
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), f"PTttabmodel_d1_l{config['num_layers']}_g{config['num_gnn_layers']}_i{config['num_int_layers']}_dp{config['dropout']}.pth")
