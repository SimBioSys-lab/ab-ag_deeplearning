...
class Attention(nn.Module):
    def __init__(
        ...
        if use_rope:
            ...
            sinusoid = torch.einsum("i , j -> i j", torch.arange(max_seq_len), inv_freq)
        ...
        if use_alibi:
            slopes = self._get_alibi_slopes(heads)
            alibi = slopes[:, None, None] * torch.arange(max_seq_len)[None, None, :]
            self.register_buffer("alibi", alibi, persistent=False)

    def _get_alibi_slopes(self, n_heads):
        def get_slopes(n):
            def power_of_2(x): return 2 ** math.floor(math.log2(x))
            m = power_of_2(n)
            slopes = torch.pow(2, -(torch.arange(1, m + 1).float()))
            if m != n:
                extra = torch.pow(2, -(torch.arange(1, 2 * (n - m) + 1, 2).float()))
                slopes = torch.cat([slopes, extra])
            return slopes
        return get_slopes(n_heads).unsqueeze(1)

    def _apply_rope(self, t, seq_len):
        sin, cos = self.sin[:seq_len], self.cos[:seq_len]  # (L, D/2)
        sin = repeat(sin, "n d -> b h n d", b=t.size(0), h=self.heads)
        cos = repeat(cos, "n d -> b h n d", b=t.size(0), h=self.heads)
        t1, t2 = t[..., ::2], t[..., 1::2]
        rot = torch.stack((t1 * cos - t2 * sin, t1 * sin + t2 * cos), dim=-1)
    ...
    def forward(self, x, mask=None, tied=False, return_attention=False):
        ...
        if self.use_rope:
            q = self._apply_rope(q, N)
            k = self._apply_rope(k, N)

        q = q * self.scale
        attn_scores = einsum("b h i d, b h j d -> b h i j", q, k)

        if self.use_alibi:
            attn_scores += self.alibi[:, :, :N]

        if tied:
            rowwise_avg = attn_scores.mean(dim=3, keepdim=True)
            scaling_factor = math.sqrt(B)
            attn_scores = (rowwise_avg / scaling_factor).expand_as(attn_scores)
...
class CoreModel(nn.Module):
    ...
    def forward(self, sequences: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(self.self_attention_layers)):
            ...
            res = x
            x_attn = self.self_attention_layers[i](x, mask=None)  # (B, L, D)
    ...
    def forward(self, sequences, padded_edges, mask=None, pairwise_repr=None,
        ...
        for idx, (row_attn, ffn, rnorm, fnorm) in enumerate(zip(
            ...
            if return_attention and idx == len(self.row_attn_layers) - 1:
                row_out, attn = row_attn(output, mask=mask,
                                         edges=None, return_attention=True, tied=tied)
                last_attn = attn
            else:
