...
for fold, (train_indices, val_indices) in enumerate(kf.split(dataset_indices), 1):
    ...
    # Load parameters
    core_params = torch.load('isicisicimodelFull10_fold5_l0_g20_i8_dp0.1_core.pth', map_location=device)
...
#    criterion = nn.CrossEntropyLoss(ignore_index=-1, weight=class_weight)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'isicisiciimodelFull4.5_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
