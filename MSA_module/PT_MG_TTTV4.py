...
from Models_NFnew import ClassificationModel
...
#    # Load parameters
#    core_params = torch.load('isicttmodelFullNew45_fold2_l0_g20_i8_dp0.1_core.pth', map_location=device)
...
#    criterion = nn.CrossEntropyLoss(ignore_index=-1, weight=class_weight)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'ittmodelNFNew45_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
