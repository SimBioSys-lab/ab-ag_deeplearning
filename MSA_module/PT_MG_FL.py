...
#    class_weight = torch.tensor([1.0, 20.0])
...
#                loss = criterion(output_seq, pt_tensor) / config['accumulation_steps']
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'imodelFull4.5FL_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
