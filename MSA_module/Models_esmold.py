...
class CoreModel(nn.Module):
    """
    CoreModel with frozen (or optionally fine-tuned) ESM-2 small embeddings.
    No linear projection – downstream blocks now work with 320-D features.
    """

    def __init__(
        self,
        vocab_size      : int,          # kept for API compatibility
        seq_len         : int,
        embed_dim       : int = None,   # ignored; will be set to 320
        num_heads       : int = 16,
        dropout         : float = 0.1,
        num_layers      : int = 1,
        ...
        # ── 1) load ESM-2 small ────────────────────────────────────────────
        self.esm, alphabet = esm.pretrained.load_model_and_alphabet_hub(esm_ckpt)
        ...
        # ── 2) build downstream blocks (now dim = 320) ─────────────────────
    ...
    # --------------------------------------------------------------------- #
    # Helper: map your 23-symbol integer alphabet → ESM alphabet
    # --------------------------------------------------------------------- #
    @staticmethod
    def _to_esm_tokens(seq_ids: torch.Tensor) -> torch.Tensor:
        """
        seq_ids : (N, L) ints in your alphabet (0-22)
        returns : (N, L) ints in ESM alphabet
        """
        mapping = torch.tensor([
             21,  4,  3,  6, 13,  8,  9, 16,  1, 14,   # A R N D C Q E G H I
              7, 15, 11, 10,  5, 19,  2, 12, 20, 18,   # L K M F P S T W Y V
             22,  0, 23                                 # X gap pad
        ], device=seq_ids.device, dtype=torch.long)
        return mapping[seq_ids]

    # --------------------------------------------------------------------- #
    def forward(self, sequences: torch.Tensor) -> torch.Tensor:
        """
        sequences : (B, 64, L) integer encoded.
        returns   : (B, 64, L, 320) tensor.
        """
        B, S, L = sequences.shape
        assert S == 64 and L == self.seq_len, "Unexpected input shape"

        # 1) ESM embeddings
        toks = self._to_esm_tokens(sequences).reshape(B * S, L)
        with torch.no_grad():                                  # frozen by default
            rep = self.esm(toks, repr_layers=[0])["representations"][0]   # (B·64,L,320)
        x = rep.view(B, S, L, self.embed_dim)                  # (B,64,L,320)

        # 2) Your axial-attention + DyM pipeline
        batch_splits = [_find_chain_splits_1d(sequences[b, 0]) for b in range(B)]

        for i in range(len(self.self_attention_layers)):
            # SA per chain
            res, x_attn = x, x.clone()
            for b in range(B):
                for s, e in batch_splits[b]:
                    seg = x[b:b+1, :, s:e, :]
                    seg = self.self_attention_layers[i](seg)
                    x_attn[b, :, s:e, :] = seg.squeeze(0)
            ...
            # FFN
            res   = x
            x_ffn = self.ffn_layers[i](x.view(B * S, L, -1)).view(B, S, L, -1)
