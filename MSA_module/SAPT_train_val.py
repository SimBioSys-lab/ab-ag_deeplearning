...
from Dataloader_SS_SASA import SequenceSAPTDataset
from Models import SAPTModel
...
# Dataset preparation
dataset = SequenceSAPTDataset(sequence_file=config['sequence_file'], sapt_file=config['sapt_file'], max_len=config['seq_len'])
...
for epoch in range(config['num_epochs']):
    ...
    for seq, sapt in train_loader:
        optimizer.zero_grad()
        seq, sapt = seq.to(device), sapt.to(device)

        # Skip batches with NaN or Inf
        if torch.isnan(seq).any() or torch.isinf(seq).any() or torch.isnan(sapt).any() or torch.isinf(sapt).any():
            print("NaN or Inf found in input; skipping batch.")
            continue

        with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
            output_sapt = model(seq)  # Model outputs a single value per sequence
            loss = criterion(output_sapt, sapt)
    ...
    val_loss = 0
    with torch.no_grad():
        for seq, sapt in val_loader:
            seq, sapt = seq.to(device), sapt.to(device)

            with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                output_sapt = model(seq)
                loss = criterion(output_sapt, sapt)
                val_loss += loss.item()
...
if best_model_state is not None:
    torch.save(best_model_state, 'best_sapt_model_with_decay_noise.pth')
