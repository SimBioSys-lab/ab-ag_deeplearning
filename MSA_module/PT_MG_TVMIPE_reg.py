...
for fold,(tr_idx,vl_idx) in enumerate(kf.split(np.arange(len(dataset))),1):
    ...
    # Load parameters
    core_params = torch.load('isMIPEreg_l1_g6_i3_do0.10_dpr0.10_lr0.0001_fold3_core.pth', map_location=device)
    ...
    ckpt=(f"isiMIPEreg_l{config['num_layers']}_g{config['num_gnn_layers']}"
