...
config = {
    ...
    'data_file':      'ihd_train_esminterfaces_1600.npz',
...
for fold,(tr_idx,vl_idx) in enumerate(kf.split(np.arange(len(dataset))),1):
    ...
    # Load parameters
    core_params = torch.load('isicIHDreg_l1_g12_i6_do0.1_dpr0.1_fold5_core.pth', map_location=device)
    ...
    ckpt=(f"isiciIHDreg_l{config['num_layers']}_g{config['num_gnn_layers']}"
