...
config = {
    ...
    'data_file': 'combined_np_train_interface10_1600.npz',
...
#    criterion = nn.CrossEntropyLoss(ignore_index=-1, weight=class_weight)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'imodelFullNew10_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
