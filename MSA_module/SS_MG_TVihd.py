...
config = {
    ...
    'data_file':      'ihd_train_esmss_1600.npz',
...
for fold,(tr_idx,vl_idx) in enumerate(kf.split(np.arange(len(dataset))),1):
    ...
    # Load parameters
    core_params = torch.load('iIHDreg_l1_g12_i6_do0.10_dpr0.10_lr0.0001_heads16_fold3_core.pth', map_location=device)
...
#        criterion = nn.CrossEntropyLoss(weight=torch.tensor([1.0,pw], device=device), ignore_index=-1)
    ...
    ckpt=(f"isIHDreg_l{config['num_layers']}_g{config['num_gnn_layers']}"
