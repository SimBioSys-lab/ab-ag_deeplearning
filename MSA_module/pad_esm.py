...
# Configuration
Threshold = 1600
output_prefix = f"pecanepi_test"

# Load input files
train_seqs = np.load("pecanepi_test_esmsequences.npz", allow_pickle=True)
interfaces = np.load("pecanepi_test_interfaces.npz", allow_pickle=True)
edges = np.load("pecanepi_test_edges.npz", allow_pickle=True)
...
#        continue
    ...
    # Pad sequence
    padded_seq = np.ones((64, Threshold), dtype=seq.dtype)
...
# Save results
np.savez_compressed(f"{output_prefix}_esmsequences_{Threshold}.npz", **padded_sequences)
np.savez_compressed(f"{output_prefix}_esminterfaces_{Threshold}.npz", **padded_interfaces)
np.savez_compressed(f"{output_prefix}_esmedges_{Threshold}.npz", **filtered_edges)
