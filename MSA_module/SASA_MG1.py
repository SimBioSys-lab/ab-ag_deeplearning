...
from Models_test import RegressionModel
...
config = {
    'batch_size': 4,
    'sequence_file': 'padded_sequences_train_filtered_2400.npz',
    'data_file': 'padded_train_sasa_filtered_2400.npz',
    'edge_file': 'padded_edges_train_filtered_2400.npz',
    'max_len': 2400,
    'vocab_size': 23,
    'embed_dim': 256,
    'num_heads': 4,
...
for fold, (train_indices, val_indices) in enumerate(kf.split(dataset_indices), 1):
    ...
    # Load parameters
    core_params = torch.load('isicimodel_fold2_l1_g1_i1_dp0.1_core.pth', map_location=device)
    ...
    for epoch in range(config['num_epochs']):
        ...
        for i, (padded_edges, sequence_tensor, pt_tensor) in enumerate(train_loader):
            ...
            if torch.isnan(pt_tensor).any() or torch.isinf(pt_tensor).any():
                ...
                continue
            mask = pt_tensor != -1  # Create mask for valid targets
            valid_targets = pt_tensor[mask]
            if valid_targets.numel() == 0:
                continue

            with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                outputs = model(sequence_tensor, padded_edges).squeeze(-1)  # Shape: [batch_size, seq_len]
                expanded_mask = mask.unsqueeze(1)
                valid_outputs = outputs[expanded_mask]
                valid_targets = valid_targets.to(torch.float)
                loss = criterion(valid_outputs, valid_targets).mean()/config['accumulation_steps']
        ...
        with torch.no_grad():
            for padded_edges, sequence_tensor, pt_tensor in val_loader:
                ...
                if valid_targets.numel() == 0:
                    continue
                with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                    outputs = model(sequence_tensor, padded_edges).squeeze(-1)
                    expanded_mask = mask.unsqueeze(1)
                    valid_outputs = outputs[expanded_mask]
                    valid_targets = valid_targets.to(torch.float)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'isiciamodel_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
