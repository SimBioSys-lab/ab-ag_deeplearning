...
config = {
    'batch_size': 4,
    'sequence_file': 'antigen_train_sequences_3000.npz',
    'data_file': 'antigen_train_interfaces_3000.npz',
    'edge_file': 'antigen_train_edges_3000.npz',
    'max_len': 3000,
    'vocab_size': 23,
    'embed_dim': 256,
    'num_heads': 16,
...
for fold, (train_indices, val_indices) in enumerate(kf.split(dataset_indices), 1):
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'mcagmodel_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
