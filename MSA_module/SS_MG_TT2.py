...
# Load parameters
core_params = torch.load('isicittmodelFullTV10_fold2_l0_g20_i8_dp0.2_core.pth', map_location=device)
...
for epoch in range(config['num_epochs']):
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), f"isicisttmodelFull10_l{config['num_layers']}_g{config['num_gnn_layers']}_i{config['num_int_layers']}_dp{config['dropout']}.pth")
