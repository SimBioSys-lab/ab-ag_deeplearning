...
config = {
    ...
    'data_file': 'global_maps_MIPE_esmtv.npz',
...
for fold, (train_indices, val_indices) in enumerate(kf.split(dataset_indices), 1):
    ...
    # Load parameters
    core_params = torch.load('isiParamodelnewesm_l1_g10_i5_do0.15_dpr0.15_lr0.0001_fold4_core.pth', map_location=device)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'isicParamodelnewesm_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_do{config["dropout"]}_dpr{config["drop_path_rate"]}_fold{fold}.pth')
