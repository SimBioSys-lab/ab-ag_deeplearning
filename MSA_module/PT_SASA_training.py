...
from Dataloader_SS_SASA import SequenceParatopeDataset, SequenceSASADataset
from Models import PTSASAModel
...
train_sasa, val_sasa = random_split(sasa_dataset, [train_size, val_size])

# Data loaders
train_loader = DataLoader(list(zip(train_paratope, train_sasa)), batch_size=config['batch_size'], shuffle=True)
val_loader = DataLoader(list(zip(val_paratope, val_sasa)), batch_size=config['batch_size'], shuffle=False)
...
criterion_sasa = nn.MSELoss(reduction='none')  # For SASA regression
...
for epoch in range(config['num_epochs']):
    ...
    for (seq_pt, tgt_pt), (seq_sasa, tgt_sasa) in train_loader:
        optimizer.zero_grad()
        ...
        with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
            sasa_output, pt_output = model(seq_pt)
    ...
    val_loss = 0
    with torch.no_grad():
        for (seq_pt, tgt_pt), (seq_sasa, tgt_sasa) in val_loader:
            seq_pt, tgt_pt = seq_pt.to(device), tgt_pt.to(device)
            seq_sasa, tgt_sasa = seq_sasa.to(device), tgt_sasa.to(device)
            ...
            with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                ...
                val_loss += (loss_sasa * 8 + loss_pt).item()
...
if best_model_state is not None:
    torch.save(best_model_state, 'best_ptsasa_model.pth')
