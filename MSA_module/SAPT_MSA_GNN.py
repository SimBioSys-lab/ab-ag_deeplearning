...
from Models_new import SAPTModel
import numpy as np
...
criterion = nn.MSELoss(reduction='mean')  # Mean reduction for scalar loss
...
for epoch in range(config['num_epochs']):
    ...
    for i, (padded_edges, sequence_tensor, ss_tensor, sasa_tensor, pt_tensor, sapt_tensor) in enumerate(train_loader):
        ...
        sequence_tensor, sapt_tensor = sequence_tensor.to(device), sapt_tensor.to(device)
        padded_edges = padded_edges.to(device)

        if torch.isnan(sapt_tensor).any() or torch.isinf(sapt_tensor).any():
            print("NaN or Inf found in input; skipping batch.")
            continue

        with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
            output = model(sequence_tensor, padded_edges)  # Output shape: [batch_size]
            sapt_tensor = sapt_tensor.view(-1)
            loss = criterion(output, sapt_tensor)
    ...
    with torch.no_grad():
        for padded_edges, sequence_tensor, ss_tensor, sasa_tensor, pt_tensor, sapt_tensor in val_loader:
            padded_edges, sequence_tensor, sapt_tensor = padded_edges.to(device), sequence_tensor.to(device), sapt_tensor.to(device)
            with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                output = model(sequence_tensor, padded_edges)
                sapt_tensor = sapt_tensor.view(-1)
                loss = criterion(output, sapt_tensor)
                val_loss += loss.item()
...
if best_model_state is not None:
    torch.save(best_model_state, f'PTSAPT_l{config["num_layers"]}_g{config["num_gnn_layers"]}_d{config["embed_dim"]}_h{config["num_heads"]}_bs{config["batch_size"]}x2.pth')
