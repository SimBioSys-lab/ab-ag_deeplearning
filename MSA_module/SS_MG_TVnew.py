...
cfg = {
    ...
    'data_file':      'cleaned_para_tv_secondary_structure_1600.npz',
...
# ───────────────────────────────── train / val
def train_one_epoch(model, loader, crit, optim_):
    model.train(); run=0.0
...
for fold,(tr_idx,vl_idx) in enumerate(kf.split(np.arange(len(ds))),1):
    ...
    # Load parameters
    core_params = torch.load('iParamodel_l0_g20_i8_do0.10_dpr0.10_lr0.0002_fold4_core.pth', map_location=device)
    ...
    name = (f"isParamodel_l{cfg['num_layers']}_g{cfg['num_gnn_layers']}"
