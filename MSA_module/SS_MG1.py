...
config = {
    ...
    'data_file': 'padded_train_secondary_structure_filtered_2400.npz',
...
for fold, (train_indices, val_indices) in enumerate(kf.split(dataset_indices), 1):
    ...
    # Load parameters
    core_params = torch.load('isiaicisiaicimodel_fold3_l1_g1_i1_dp0.1_core.pth', map_location=device)
...
#    criterion = nn.CrossEntropyLoss(ignore_index=-1,weight=class_weights_tensor)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'isiaicisiaicismodel_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
