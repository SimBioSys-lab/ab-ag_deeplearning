...
# =========================
with open('para_newesm_isicisi_files', 'r') as f:
...
# =========================
try:
    # Only if DyM is exported by Models_fullnew
...
# We infer mask = (output / input).mean(-1, keepdim=True)
# =========================
dym_batch = defaultdict(list)  # cleared per batch

def dym_hook(module, inputs, output):
    # inputs: tuple, inputs[0] is x
    x = inputs[0]
    # avoid divide-by-zero, keep mask shape (..., 1)
...
for model_file in models:
    ...
    dym_dir = f"{base_name}_dym_masks"
    attn_dir = f"{base_name}_attn"
    ...
    with open(f"{model_file}_predictions.csv", "w", newline="") as f:
        ...
        with torch.no_grad():
            for padded_edges, sequence_tensor, pt_tensor in test_loader:
                ...
                # -------- Save last_attention for this sample --------
                sample_key = interface_keys[sample_idx] if sample_idx < len(interface_keys) else f"sample_{sample_idx}"
                np.save(
                    os.path.join(attn_dir, f"{sample_key}.npy"),
                    last_attention.detach().cpu().numpy()
                )

                # -------- Save DyM masks from this batch via dym_batch --------
                # (batch_size==1 ⇒ one mask per layer per batch)
                for layer_name, masks in dym_batch.items():
                    if len(masks) == 0:
                        continue
                    # save the first (and only) mask for this layer for this batch
                    torch.save(masks[0], os.path.join(dym_dir, f"{layer_name}__{sample_key}.pt"))
                dym_batch.clear()  # IMPORTANT: clear per batch
