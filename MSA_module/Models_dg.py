...
class CoreModel(nn.Module):
    def __init__(self, vocab_size, seq_len, embed_dim, num_heads, dropout, num_layers):
        ...
        # Create a learnable mask for each self-attention layer.
        # Each mask is computed via a fully connected layer mapping embed_dim -> 1.
    ...
    def forward(self, sequences):
        ...
        for i, (attn_layer, norm_layer) in enumerate(zip(self.self_attention_layers, self.norm_layers)):
            residual = output
            attn_out = attn_layer(output, mask=None)  # [batch_size, seq_len, embed_dim]
            attn_out = self.dropout(attn_out)
            # Compute the learnable, residue-dependent mask: shape [batch, seq_len, 1].
            mask_val = torch.sigmoid(self.mask_layers[i](attn_out))
            # Apply the mask (broadcast along embed_dim).
            attn_out = attn_out * mask_val
...
class CGModel(nn.Module):
    ...
    Assumes that the CoreModel outputs a tensor of shape [batch_size, 1, seq_len, embed_dim].
    """
    def __init__(self, vocab_size, seq_len, embed_dim, num_heads, dropout, num_layers, num_gnn_layers):
        ...
        # Core Model for sequence embeddings (assumed to output [batch_size, 1, seq_len, embed_dim])
        self.core_model = CoreModel(vocab_size, seq_len, embed_dim, num_heads, dropout, num_layers)
        self.dropout = nn.Dropout(dropout)

        # Positional embedding: shape [seq_len, embed_dim]
        ...
        # Residue-dependent mask for FC output: mapping embed_dim -> 1.
        self.fc_mask = nn.Linear(embed_dim, 1)

        # GAT layers for graph-based sequence refinement.
        self.gnn_layers = nn.ModuleList([
            GATConv(embed_dim, embed_dim, heads=1, concat=False)
        ...
        # Row attention layers for each GNN layer.
        ...
        # FC layers to compute contact logits from the row attention output.
        self.fc_layers = nn.ModuleList([
            nn.Linear(embed_dim, 1) for _ in range(num_gnn_layers)
        ])
        # Default contact threshold.
        self.contact_threshold = 0.5

    def forward(self, sequences, padded_edges, threshold=None):
        """
        Args:
            sequences (torch.Tensor): Input sequences of shape [batch_size, 1, seq_len].
            padded_edges (torch.Tensor): Padded edge indices of shape [batch_size, 2, max_edges].
            threshold (float, optional): Threshold to decide contacts from row attention.
                                         If None, defaults to self.contact_threshold.
        Returns:
        ...
        # Obtain core embeddings (already [batch_size, 1, seq_len, embed_dim]).
        output = self.core_model(sequences)  
        output = self.dropout(output)
        # Take only the first row. now [batch_size, seq_len, embed_dim].
        output = output[:, 0, :, :]
        # Add positional embeddings.
        pos_emb = self.positional_embeddings.unsqueeze(0).expand(batch_size, -1, -1)
        output = output + self.dropout(pos_emb)
        ...
        # Flatten for GNN layers: [batch_size * seq_len, embed_dim]
        embedding_flat = embedding.view(-1, embedding.shape[-1])
        ...
        for i in range(batch_size):
            ...
            batch_list.append(torch.full((valid_edges.shape[1],), i, dtype=torch.long, device=valid_edges.device))
        ...
        for i, (gnn_layer, norm_layer, row_attn, fc_layer, gnn_mask_layer) in enumerate(zip(
                self.gnn_layers, self.norm_layers, self.row_attn_layers, self.fc_layers, self.gnn_mask_layers)):
            # Apply the GNN layer.
            gnn_out = gnn_layer(embedding_flat, edge_index)  # [batch_size * seq_len, embed_dim]
            gnn_out = self.dropout(gnn_out)
            # Reshape for row attention: [batch_size, 1, seq_len, embed_dim].
            gnn_out_reshaped = gnn_out.view(batch_size, 1, seq_len, -1)
            # Apply row attention; assume it returns (row_out, attn) where attn is [batch_size, seq_len, seq_len].
            row_out, attn = row_attn(gnn_out_reshaped, return_attention=True)
            # Compute contact logits from row attention output.
            contact_logits = fc_layer(row_out)  # [batch_size, seq_len, 1]
            contact_prob = torch.sigmoid(contact_logits).squeeze(-1)  # [batch_size, seq_len]
            # Update edge_index for each graph in the batch.
            new_edge_indices = []
            for b in range(batch_size):
                attn_b = attn[b]  # [seq_len, seq_len]
                j, k = torch.where(attn_b > threshold)
                new_edge_indices.append(torch.stack([j, k], dim=0))
            if new_edge_indices:
                new_edge_index = torch.cat(new_edge_indices, dim=1)
            else:
                new_edge_index = edge_index  # fallback
            edge_index = new_edge_index

            # Compute a learnable mask for the GNN output.
            gnn_mask_val = torch.sigmoid(gnn_mask_layer(gnn_out))  # [batch_size*seq_len, 1]
            gnn_out = gnn_out * gnn_mask_val
            # Residual connection and normalization.
            embedding_flat = norm_layer(gnn_out + embedding_flat)
        
        # Reshape back to [batch_size, 1, seq_len, embed_dim].
...
class MCModel(nn.Module):
    def __init__(self, vocab_size, seq_len, embed_dim, num_heads, dropout, 
        ...
        # For each row attention layer, create a fully connected layer that computes a per-residue mask.
        # It maps from embed_dim to 1. The output is then passed through a sigmoid.
    ...
    def forward(self, sequences, padded_edges, mask=None, pairwise_repr=None, return_attention=False, tied=False):
        ...
        for idx, (row_attn, norm) in enumerate(zip(self.row_attn_layers, self.row_norm_layers)):
            ...
            # Compute the mask: shape [batch, seq_len, 1] from row_out of shape [batch, seq_len, embed_dim]
            mask_val = torch.sigmoid(self.row_mask_layers[idx](row_out))
            # Apply the mask elementwise (broadcasted along the embed_dim dimension)
            row_out = row_out * mask_val
            # Add the residual connection and then normalize.
