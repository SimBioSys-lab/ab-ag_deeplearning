...
cfg = {
    ...
    'dropout':    0.20, 'num_layers': 0,
    'num_gnn_layers': 20, 'num_int_layers': 8,
    'drop_path_rate': .15,'num_classes': 2,
    ...
    # pos-weight schedule (constant 1:1 here)
    'weight_start': 1.0, 'weight_end': 1.0,
...
for fold,(tr_idx,vl_idx) in enumerate(kf.split(np.arange(len(ds))),1):
    ...
    # (Optional) load pre-trained core weights
    core_params = torch.load('PPI_model_l0_g20_i8_do0.15_dpr0.15_lr0.0002_fold1_core.pth',
                             map_location=device)
    model_state = model.state_dict(); model_state.update(core_params)
    ...
    for epoch in range(cfg['num_epochs']):
        # constant pos-weight =1
        crit = nn.CrossEntropyLoss(weight=torch.tensor([1.0,1.0], device=device),
                                   ignore_index=-1)
    ...
    final = (f"iMIPE_model_l{cfg['num_layers']}_g{cfg['num_gnn_layers']}"
