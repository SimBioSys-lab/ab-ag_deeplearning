...
pretrained_paths = [
    f"PT_l{config["num_layers"]}_g{config["num_gnn_layers"]}_d256_h16_bs4x2_core.pth",
    f"PTSS_l{config["num_layers"]}_g{config["num_gnn_layers"]}_d256_h16_bs4x2_core.pth",
    f"PTSASA_l{config["num_layers"]}_g{config["num_gnn_layers"]}_d256_h16_bs4x2_core.pth",
    f"PTSAPT_l{config["num_layers"]}_g{config["num_gnn_layers"]}_d256_h16_bs4x2_core.pth"
]
pretrained_weights = [torch.load(path, map_location=device) for path in pretrained_paths]
...
for fold, (train_indices, val_indices) in enumerate(kf.split(dataset_indices), 1):
    ...
    # Initialize trainable weights for pretrained parameters
    a = nn.Parameter(torch.tensor(0.25, requires_grad=True, device=device))
    b = nn.Parameter(torch.tensor(0.25, requires_grad=True, device=device))
    c = nn.Parameter(torch.tensor(0.25, requires_grad=True, device=device))
    d = nn.Parameter(torch.tensor(0.25, requires_grad=True, device=device))

    def normalized_weights():
        """Compute softmax-normalized weights."""
        return torch.softmax(torch.stack([a, b, c, d]), dim=0)

    def apply_pretrained_params(model_state_dict):
        """Blend pretrained parameters."""
        weights = normalized_weights()
        for key in model_state_dict.keys():
            if key in pretrained_weights[0]:
                model_state_dict[key] = (
                    weights[0] * pretrained_weights[0][key]
                    + weights[1] * pretrained_weights[1][key]
                    + weights[2] * pretrained_weights[2][key]
                    + weights[3] * pretrained_weights[3][key]
                )
        return model_state_dict

    # Apply the blended parameters to the model
    model.load_state_dict(apply_pretrained_params(model.state_dict()), strict=False)

    # Loss function, optimizer, scaler, scheduler
    criterion = nn.CrossEntropyLoss(ignore_index=-1)
    optimizer = optim.Adam(list(model.parameters()) + [a, b, c, d], lr=config['learning_rate'])
    scaler = GradScaler()
    ...
    for epoch in range(config['num_epochs']):
        ...
        for i, (padded_edges, sequence_tensor, _, _, pt_tensor, _) in enumerate(train_loader):
            ...
            with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                ...
                loss = criterion(output_seq.view(-1, config['num_classes']), pt_tensor.view(-1)) / config['accumulation_steps']
        ...
        val_loss = 0
        with torch.no_grad():
            for padded_edges, sequence_tensor, _, _, pt_tensor, _ in val_loader:
                padded_edges, sequence_tensor, pt_tensor = padded_edges.to(device), sequence_tensor.to(device), pt_tensor.to(device)
                with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                    output_seq = model(sequence_tensor, padded_edges)
                    loss = criterion(output_seq.view(-1, config['num_classes']), pt_tensor.view(-1))
                    val_loss += loss.item()
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'PT2_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_d{config["embed_dim"]}_h{config["num_heads"]}_bs{config["batch_size"]}x2.pth')
