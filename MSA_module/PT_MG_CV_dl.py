...
config = {
    'batch_size': 4,
    'sequence_file': 'padded_sequences_3000.npz',
    'data_file': 'padded_interfaces_3000.npz',
    'edge_file': 'padded_edges_3000.npz',
    'max_len': 3000,
    'vocab_size': 23,
    'embed_dim': 256,
    'num_heads': 16,
...
class AUC_PRLoss(nn.Module):
    def __init__(self):
        super(AUC_PRLoss, self).__init__()
    def forward(self, logits, targets):
        ...
        pairwise_diffs = pos_probs.unsqueeze(1) - neg_probs.unsqueeze(0)
        pairwise_loss = 1 - torch.sigmoid(pairwise_diffs)
...
#    criterion = nn.CrossEntropyLoss(ignore_index=-1)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'mcdlmodel_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}.pth')
