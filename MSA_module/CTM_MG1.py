...
config = {
    ...
    'data_file': 'global_maps_train_aligned_2400.npz',
...
for fold, (train_indices, val_indices) in enumerate(kf.split(dataset_indices), 1):
    ...
    # Load parameters
    core_params = torch.load('isiaicisiaimodel_fold5_l1_g1_i1_dp0.1_core.pth', map_location=device)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'isiaicisiaicmodel_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
