...
class CoreModel(nn.Module):
    """
    CoreModel that consumes integer tokens already in the ESM alphabet
    (shape: B x 64 x L). It calls a small ESM-2 model to obtain token
    embeddings (dim=320) and then applies your downstream axial-attn + FFN stack.
    """

    def __init__(
        self,
        vocab_size      : int,          # unused, kept for API compatibility
        seq_len         : int,
        embed_dim       : int = None,   # ignored; set from ESM
        num_heads       : int = 16,
        dropout         : float = 0.1,
        num_layers      : int = 1,
        drop_path_rate  : float = 0.1,
        esm_ckpt        : str  = "esm2_t6_8M_UR50D",
        freeze_esm      : bool = True,   # <-- default now NOT frozen
    ):
        super().__init__()

        # Load ESM model + alphabet
        self.esm, _alphabet = esm.pretrained.load_model_and_alphabet_hub(esm_ckpt)
        self.embed_dim = self.esm.embed_dim    # e.g., 320
        ...
        self.norm_layers     = nn.ModuleList([nn.LayerNorm(self.embed_dim) for _ in range(num_layers)])
        self.ffn_norm_layers = nn.ModuleList([nn.LayerNorm(self.embed_dim) for _ in range(num_layers)])
        self.dropout         = nn.Dropout(dropout)

        self.attn_dym = nn.ModuleList([DyM(self.embed_dim) for _ in range(num_layers)])
        self.ffn_dym  = nn.ModuleList([DyM(self.embed_dim) for _ in range(num_layers)])
    ...
    # --- helper: where does ESM live? (defensive; cheap) --------------------
    def _esm_device(self) -> torch.device:
        # Try param, fallback buffer, else CPU.
        for p in self.esm.parameters():
            return p.device
        for b in self.esm.buffers():
            return b.device
    ...
    def forward(self, toks: torch.Tensor) -> torch.Tensor:
        """
        toks : (B, 64, L)   *already* ESM tokens  (uint16 / int64)
        """
        B, S, L = toks.shape
        assert S == 64 and L == self.seq_len
    
        # 1) ensure on the right device – will be a view, not a copy, if already correct
        toks = toks.to(self._esm_device(), non_blocking=True)
    
        # 2) Optional: run ESM in half-precision to halve memory
        #    (keep in fp32 if you fine-tune and need maximum stability)
        if not self.freeze_esm:                       # fine-tuning
            rep = self.esm(toks.view(B*S, L), repr_layers=[0])["representations"][0]
        else:                                         # frozen ⇒ no grad ⇒ we can safely use fp16
            with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16):
                rep = self.esm(toks.view(B*S, L), repr_layers=[0])["representations"][0]
            rep = rep.float()     # cast back to fp32 for downstream maths if you prefer
    
        x = rep.view(B, S, L, self.embed_dim)         # (B,64,L,320)
        # Downstream axial-attn + DyM + FFN (no per-chain splitting)
        for i in range(len(self.self_attention_layers)):
            # Attention branch
            res = x
            x_attn = self.self_attention_layers[i](x)   # expect same shape (B,64,L,320)
            ...
            # FFN branch
            res = x
            # collapse (B*64, L, D) for FFN; then reshape back
            x_ffn = self.ffn_layers[i](x.view(B * S, L, self.embed_dim))
            x_ffn = x_ffn.view(B, S, L, self.embed_dim)
