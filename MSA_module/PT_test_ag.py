...
test_config = {
    'batch_size': 1,
    'sequence_file': 'antigen_test_sequences_3000.npz',
    'data_file': 'antigen_test_interfaces_3000.npz',
    'edge_file': 'antigen_test_edges_3000.npz',
    'max_len': 3000,
    'vocab_size': 23,
    'num_classes': 2
}
...
for model_file in models:
    ...
    # Initialize metrics for Antigen only
    metrics = {"Antigen": {"true": [], "pred": [], "probs": []}}
    ...
    with open(f"{model_file}_predictions.csv", "w", newline="") as f:
        ...
        with torch.no_grad():
            for padded_edges, sequence_tensor, pt_tensor in test_loader:
                ...
                # Process metrics for antigen chains
                for i in range(len(EOCs) - 1):
                    start, end = EOCs[i], EOCs[i + 1]
                    mask = pt_tensor[0, start:end] >= 0

                    if mask.sum().item() == 0:
                        print(f"No valid positions for AGchain_{i}. Skipping...")
                        continue
                    ...
                    for t, p, pr in zip(true_labels, predicted_labels, probabilities):
                        writer.writerow([f"AGchain_{i}", t, p, pr])

    # Calculate and print final metrics for Antigen
    true = metrics["Antigen"]["true"]
    pred = metrics["Antigen"]["pred"]
    probs = metrics["Antigen"]["probs"]
