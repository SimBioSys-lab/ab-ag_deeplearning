...
#        print(f'padded_edges.shape = {padded_edges.shape}', flush=True)
        ...
        with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
            ...
            pt_tensor = pt_tensor.view(-1)  # Flatten targets
    ...
    with torch.no_grad():
        for padded_edges, sequence_tensor, ss_tensor, sasa_tensor, pt_tensor, sapt_tensor in val_loader:
            padded_edges, = padded_edges.to(device)  # Move batched graph to device
            sequence_tensor, pt_tensor = sequence_tensor.to(device), pt_tensor.to(device)
            with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                output_seq = model(sequence_tensor, batched_graph)  # Pass the sequence tensor and batched graph to the model
                output_seq = output_seq.view(-1, config['num_classes'])  # Reshape for loss computation
                pt_tensor = pt_tensor.view(-1)  # Flatten targets
...
if best_model_state is not None:
    torch.save(best_model_state, f'model_l{config['num_layers']}_g{config['num_gnn_layers']}_d{config['embed_dim']}_h{config['num_heads']}_bs{config['batch_size']}x2.pth')
