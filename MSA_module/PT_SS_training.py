...
from Dataloader_SS_SASA import SequenceParatopeDataset, SequenceSecondaryStructureDataset
from Models import PTSSModel
...
train_ss, val_ss = random_split(ss_dataset, [train_size, val_size])

# Data loaders
train_loader = DataLoader(list(zip(train_paratope, train_ss)), batch_size=config['batch_size'], shuffle=True)
val_loader = DataLoader(list(zip(val_paratope, val_ss)), batch_size=config['batch_size'], shuffle=False)
...
criterion_ss = nn.CrossEntropyLoss(ignore_index=-1)  # For secondary structure classification
...
for epoch in range(config['num_epochs']):
    ...
    for (seq_pt, tgt_pt), (seq_ss, tgt_ss) in train_loader:
        optimizer.zero_grad()
        ...
        with autocast(device_type='cuda', enabled=torch.cuda.is_available()):
            ss_output, pt_output = model(seq_pt)
    ...
    val_loss = 0
    with torch.no_grad():
        for (seq_pt, tgt_pt), (seq_ss, tgt_ss) in val_loader:
            seq_pt, tgt_pt = seq_pt.to(device), tgt_pt.to(device)
            seq_ss, tgt_ss = seq_ss.to(device), tgt_ss.to(device)
...
if best_model_state is not None:
    torch.save(best_model_state, 'best_ptss_model.pth')
