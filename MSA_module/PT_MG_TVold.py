...
def custom_collate_fn(batch):
    ...
    for e in edges:
        ...
        pad[:, :e.shape[0]] = torch.as_tensor(e.T, dtype=torch.long)
        pads.append(pad)
...
for fold,(tr_idx,vl_idx) in enumerate(kf.split(np.arange(len(dataset))),1):
    ...
    model = ClassificationModel(
        ...
        num_classes=2, drop_path_rate=config['drop_path_rate']).to(device)
    ...
    ckpt=(f"imodel_l{config['num_layers']}_g{config['num_gnn_layers']}"
