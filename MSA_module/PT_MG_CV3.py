...
from Models_NM import ClassificationModel
...
config = {
    ...
    'data_file': 'combined_np_train_interface4.5_1600.npz',
...
#    # Load parameters
#    core_params = torch.load('isiaicisiaicmodel_fold2_l1_g1_i1_dp0.1_core.pth', map_location=device)
...
#    class_weight = torch.tensor([1.0, 50.0])
...
#    criterion = nn.CrossEntropyLoss(ignore_index=-1, weight=class_weight)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, f'imodelNM4.5_fold{fold}_l{config["num_layers"]}_g{config["num_gnn_layers"]}_i{config["num_int_layers"]}_dp{config["dropout"]}.pth')
